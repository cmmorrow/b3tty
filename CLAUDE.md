...
4. `GET /theme?name=<dark|light>` → `themeHandler` returns a `themePaletteResponse` JSON object shaped for the `B3ttyThemeSelector` component: `bg`, `fg`, `selBg` (from the `selection-background` key), `normal` (8-element array in ANSI display order), and `bright` (8-element array in ANSI display order); returns 405 for non-GET methods and 400 for unknown theme names; both error cases are logged at `[WARN ]`; color arrays follow the order: black, red, yellow, green, cyan, blue, magenta, white
...
- `saveConfigHandler` enforces the same `Sec-Fetch-Site` CSRF check; it also wraps `r.Body` with `io.LimitReader(r.Body, 4096)` before JSON decoding to cap request body size
