...
- `cmd/` — CLI layer using Cobra + Viper
  ...
  - `settings.go` — `b3tty settings` subcommand group: `get` (prints current settings from Viper config in tabular form), `set` (updates one or more settings; only changed flags are applied; after persisting calls `notifyRunningServer` which best-effort POSTs to `/settings`; warns when port/no-auth/no-browser/auto-resize are changed and require restart), `edit` (opens `conf.yaml` in `$VISUAL`/`$EDITOR`/`vi`); `postToRunningServer` and `notifyRunningServer` are helpers used by `settings set` and `theme set`
  - `theme.go` — `b3tty theme` subcommand group: `list` (shows built-in and user-defined themes, marks active theme with `*`), `set <name>` (sets active theme in config via `UpdateThemeInConfig`; also best-effort POSTs to `/theme-config` to apply live)
- `src/` — core server logic
  ...
  - `theme_handler.go` — `themePaletteHandler`, `themeConfigHandler`, `addThemeHandler`, `editThemeHandler`; `builtinThemes` map and all `default_themes/*.json` embeds; `defaultDarkTheme`, `defaultLightTheme`; exported `GetBuiltinThemeNames()` and `GetBuiltinTheme(name)` functions used by the `b3tty theme` CLI subcommand
...
- `buildTermOptions` always passes `rows`/`cols` from config directly (no longer conditional on truthy); handles an empty `fontFamily` gracefully — when empty it omits the leading comma
- `applyPageStyles` guards against an empty `fontFamily` when setting `--b3tty-font-family`
- `applyTerminalSettings(t, term, config)` (exported) applies settings broadcast from the server: updates cursorBlink, fontFamily, fontSize, and CSS custom properties; when `config.autoResize` is false also calls `term.resize(t.columns, t.rows)` live; always updates `config` fields to keep state in sync
- `applyThemeBroadcast(name, themeResp, term, menuBar, activeTheme)` (exported) applies a theme pushed over WebSocket by the server (e.g. from `b3tty theme set` CLI): updates xterm.js theme, page background styles, and menu bar colors; sets transparent background when `hasBackgroundImage`
...
- All three runtime theme-apply paths (`handleThemeChange`, `handleThemeSelected`, `handleThemeEdited`, `applyThemeBroadcast`) zero out `builtTheme.background` via `withAlpha(..., 0)` when `hasBackgroundImage` is true, so xterm renders transparently over the background image
