...
- `src/` — core server logic
  ...
  - `settings_handler.go` — `settingsHandler`; `GET /settings` returns live `ts.Server` + `ts.Client` state as `settingsConfigResponse`; `POST /settings` validates port and font-size, applies terminal fields to `ts.Client` in memory, persists all fields via `SaveSettingsToConfig`; server fields (port, noAuth, noBrowser) are persisted but NOT applied to the running server — they require a restart; enforces same-origin CSRF on POST
  ...
  - `models.go` — data structs: `Client`, `Server`, `TLS`, `Profile`, `Theme`, `TermConfig`, `CSPHeader`, `CSPHeaders`, `themePaletteResponse`, `themeConfigResponse`, `profileConfigResponse`, `editProfileResponse`, `settingsServerConfig`, `settingsTerminalConfig`, `settingsConfigResponse`; `Theme.toColorMap()` converts the struct back to a hyphenated key map for use with `UpdateThemeInConfig`/`SaveThemeToConfig`, skipping `BackgroundImage`
...
11. `GET /settings` → `settingsHandler`; returns `settingsConfigResponse` with live `ts.Server` (port, noAuth, noBrowser) and `ts.Client` (fontFamily, fontSize, cursorBlink, rows, columns) state; no CSRF check (read-only); `POST /settings` validates port (1–65535) and fontSize (≥1), defaults empty fontFamily to `DEFAULT_FONT_FAMILY`, applies terminal fields to `ts.Client` in memory, persists all fields via `SaveSettingsToConfig`, and echoes back the live server state (server fields are not applied to the running process); requires same-origin `Sec-Fetch-Site` for POST
...
- `buildTermOptions` handles an empty `fontFamily` gracefully — when empty it falls back to the monospace stack without prepending a leading comma
- `applyPageStyles` likewise guards against an empty `fontFamily` when setting `--b3tty-font-family`
- `handleSettingsEdited(e, term, config)` applies saved terminal settings to the live xterm.js instance (cursorBlink, fontFamily, fontSize) and updates the CSS custom properties; rows/columns are not applied live — they persist to the config file and take effect on restart
...
- Two tabs: **Server** (Port, Disable Authentication toggle, Disable Auto-Open Browser toggle) and **Terminal** (Font Family, Font Size, Cursor Blink, Rows, Columns)
...
- Restart notes shown conditionally: server-tab note when any server field is modified; terminal-tab note when Rows or Columns change (font/cursor apply immediately; rows/columns require a PTY restart)
